...
export abstract class SourcemapSession extends LoggingDebugSession {
  ...
  //   const commonArgs = await this.getArguments();
  //   if (commonArgs.localRoot) { return path.relative(commonArgs.localRoot, localFile); }
  //   return localFile;
  // }
  ...
  //     if (!unmappedPositions || !unmappedPositions.length) { throw new Error('map failed'); }
  ...
  //       if (relativeFile !== sm.file) { continue; }
  ...
  //       if (original.line === null || original.column === null || original.source === null) { throw new Error("unable to map"); }
